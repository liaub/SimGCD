...
class recommend_sequence():
    def __init__(self, flag):
        ...
        self.dataset_dir = "./data/" + self.dataset
        if flag == 'g':
            self.train_data = self.load_datasets('train_data.pkl')
            self.test_data = self.load_datasets('test_data.pkl')
            self.corpus_verb_mapping_list = []
            self.convert_sequences('train')
            self.convert_sequences('test')
            self.generate_corpus_verb_mapping()
            self.corpus = self.load_corpus('corpus_verb_mapping.pkl')
            self.event_ids = [_ for _ in range(len(self.corpus))]
        else:
            self.train_data = self.load_sequences('train_sequence.pkl')
...
if __name__ == '__main__':
    ...
    recommend = recommend_sequence('g')  # g代表数据生成, r代表读取
    # recommend.generate_datasets('train')
